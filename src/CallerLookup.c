...
static void ReplaceContactsTokens( AddrDBRecordPtr addrRecordP, Char* str, UInt16 strLen )
{
	...
	if ( addrRecordP )
	{			
		if ( addrRecordP->fields[firstName] )
			TxtReplaceStr( str, strLen, addrRecordP->fields[firstName], 2 ); // ^2
		else
			TxtReplaceStr( str, strLen, " ", 2 );

		if ( addrRecordP->fields[name] )
			TxtReplaceStr( str, strLen, addrRecordP->fields[name], 3 ); // ^3
		else
			TxtReplaceStr( str, strLen, " ", 3 );

		if ( addrRecordP->fields[title] )
			TxtReplaceStr( str, strLen, addrRecordP->fields[title], 4 );
		else
			TxtReplaceStr( str, strLen, " ", 4 );
		
		if ( addrRecordP->fields[company] )
			TxtReplaceStr( str, strLen, addrRecordP->fields[company], 5 );
		else
	...
	{	
		if ( ( record.fields[note] ) && ( StrLen( record.fields[note] ) ) )
		{
			...
			if ( ( pStartLocation ) && ( pEndLocation ) )
			{
				...
				// ReplaceDateTimeTokens( txtStr, txtStrLen ); // Executed in phn event handler
						...
						char* contactsNote, UInt16 contactsNoteSize )
{
    ...
    if ( pNotifyParams && pLookupParams )
	{
	    ...
	    if( ( !error ) && ( pLookupParams->resultString ) && ( StrLen( pLookupParams->resultString ) ) )
	    {
	    	...
	    	if ( contactsNote )
	    	{
				AddrDBGetNote( pLookupParams->recordNum, pLookupParams->uniqueID, contactsNote, contactsNoteSize );    
	    	}
